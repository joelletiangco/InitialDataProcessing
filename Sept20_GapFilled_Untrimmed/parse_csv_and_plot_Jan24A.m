% SEPT 20th, 2023 - update
    % updated to add a 4th order low pass butterworth filter with cut off
    % frequency of 10 Hz

function data = parse_csv_and_plot(files, optsForce, title, dir)
...
%% create file folder for trimmed ata to be written to
%rip subjectName dir and tyep dir (ex shoesExcel), basically just need to
%get rid of dot at front

subdir = extractAfter(dir, '.');
%create directory for files to be written to
mkdir(['./trimmed_excel' subdir]) 

%% define variables of interest here.
%% FIXED FINDING WHERE THESE VARIABLES ARE
% ADD MORE VARS YOU WANT TO FIND HERE
vars2find = {'RAnkleAngles', 'LAnkleAngles', 'RKneeAngles', 'LKneeAngles', 'RHipAngles', 'LHipAngles'};
...
for i = 1:1:length(files.force)
    clear temp non_z force_trim mean_x mean_y mean_z
    temp = readtable([dir files.force(i).name], optsForce);
    
    %% find indices of non zero elements in force
    %index where force begins to be applied will be non_z(1) and where it
    %stops being applied will be non_z(end)
%     [dir files.force(i).name]
    %non_z = find(temp.Fz);
    
    %% declaring filtering to use on messed daata
    %% this is a force treadmill paper https://www-sciencedirect-com.subzero.lib.uoguelph.ca/science/article/pii/S0966636207001804 but let's just try this for now
%     SEPT 20th, 2023: Adding a filter 
% 4 is for 4th order, the 10Hz / sampling frequency / 2 is the normalized
% cut off frequency
% (filtfilt applies a zero lag filter) 
%     [b, a] = butter(4, 10/(1000/2), 'low');
    %% making robust to non-zeroed FP
    %if non_z(1)<100
    untrimmed_F(i).Force = array2table([temp.Fx temp.Fy temp.Fz], 'VariableNames', {'Fx', 'Fy', 'Fz'});  
%    Sept 20th: Updating this to take the mean from the first 2 seconds (2000 frames)
%     Step 1: FILTER
%     temp.Fx = filtfilt(b, a, temp.Fx);
%     temp.Fy = filtfilt(b, a, temp.Fy);
%     temp.Fz = filtfilt(b, a, temp.Fz);

%     Step 2: Find mean
    mean_x = mean(temp.Fx(1:2000));
    mean_y = mean(temp.Fy(1:2000));
    mean_z = mean(temp.Fz(1:2000));
%     
%     Step 3: subtract the mean
    temp.Fx = temp.Fx-mean_x;
    temp.Fy = temp.Fy-mean_y;
    temp.Fz = temp.Fz-mean_z;
    
    
    non_z = find(temp.Fz<-1);
%     non_z = find(temp.Fz<-7);
%     non_z(1)
%     non_z(end)

    % find first value of non_z that is greater than 3460
%     non_z(non_z>3460);
%     fprintf('non z greater than 3460')
%     non_z(1)
%     non_z(end)
    
    % this will store the trimmed data in a table, so that var names are
    % there for pandas. Trims all columns in accordance to when GRF is
    % starts and stops being applied
    force_trim = temp([non_z(1):non_z(end)], :);
    %write this force data that has been trimmed to a csv in new trimmed_excel folder, otherwise keep name same, include headers
    writetable(force_trim, ['./trimmed_excel' subdir files.force(i).name])
    
    data(i).Force = array2table([force_trim.Fx force_trim.Fy force_trim.Fz], 'VariableNames', {'Fx', 'Fy', 'Fz'});
    data(i).COP = array2table([force_trim.Cx force_trim.Cy force_trim.Cz], 'VariableNames', {'Cx', 'Cy', 'Cz'});
    data(i).M = array2table([force_trim.Mx force_trim.My force_trim.Mz], 'VariableNames', {'Mx', 'My', 'Mz'});
    ...
    clear vicon_temp vicon_trim
    vicon_temp = readtable([dir files.vicon(i).name], opts);
    %using non_z as found in the force data, trim the vicon data, divide by
    %5 because sample rate is 200/. 
%     round(non_z(1)/5)   
    if round(non_z(1)/5) < 1
        fprintf('index to start is 0, not compatible w matlabs index 1 starts')
    vicon_trim = vicon_temp([round(non_z(1)/5):round(non_z(end)/5)], :);
    %write this vicon data that has been trimmed to a csv in new trimmed_excel folder, otherwise keep name same, include headers
    writetable(vicon_trim, ['./trimmed_excel/' dir '/' files.vicon(i).name])
...
%     sgtitle([title 'GRF'])
%     subplot(3,1,2)
%     plot(-1.*untrimmed_F(i).Force.Fy)
%     hold on
%     xlabel('Frame')
%     ylabel('GRF Y (N)')
% 
%     subplot(3,1,3)
%     plot(-1.*untrimmed_F(i).Force.Fz)
%     hold on
%     xlabel('Frame')
%     ylabel('GRF Z (N)')
% end
