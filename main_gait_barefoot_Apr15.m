...
% for i = 1:1:length(subject)
    ...
    %% set up file lists for BAREFOOT data -> could probably do this in a more efficient way...
    SubjectDir = ['./' subject{i} '/BarefootExcel/'];
...
%     opts = detectImportOptions([SubjectDir S_walk_L_files.force(1).name]);
    ...
    R_walk_S = parse_and_plot(S_walk_R_files, opts, [subject{i} ' Barefoot Walk Right Side'], SubjectDir);
    L_walk_S = parse_and_plot(S_walk_L_files, opts, [subject{i} ' Barefoot Walk Left Side'], SubjectDir);

%% pause to look at barefoot data
fprintf([subject{i} ' Barefoot Data \n'])
stop = 'here';
% close all barefoot graphs
close all
